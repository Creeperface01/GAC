...
class GTSynapseCheatPlayer11(interfaz: SourceInterface, entry: SynapseEntry, clientID: Long?, ip: String, port: Int) : GTSynapsePlayer11(interfaz, entry, clientID, ip, port), ICheatPlayer {
    ...
    override val acData = ACData(this)
    ...
    override fun handleDataPacket(packet: DataPacket) {
        ...
        if (packet.pid() == ProtocolInfo.PLAYER_ACTION_PACKET) {
            if ((packet as PlayerActionPacket).action == PlayerActionPacket.ACTION_JUMP.toInt()) {
    ...
    override fun handleMovePacket(packet: cn.nukkit.network.protocol.MovePlayerPacket) {
        handleMovePacket(MovePlayerPacket().fromDefault(packet))
    }
